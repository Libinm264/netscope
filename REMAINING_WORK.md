...
| N3 | **PostgreSQL wire-protocol decoding — 🟢 v1 shipped (2026-08-08).** The original founding spec (recovered from [SPEC.docx](SPEC.docx)) envisioned DB wire-protocol decoding for PostgreSQL, MySQL, Redis, Kafka, AMQP as a core desktop feature; this delivers the first protocol end-to-end: capture → decode → hub ingest. Simple-query protocol only (`Q`/`C`/`E` messages) — extended/prepared-statement protocol and MySQL/Redis/Kafka/AMQP remain unbuilt (see follow-ups below). | L | [agent/crates/parser/src/postgres.rs](agent/crates/parser/src/postgres.rs) (new, 11 unit tests, all passing), wired into [agent/crates/parser/src/session.rs](agent/crates/parser/src/session.rs) (port 5432 dispatch), [agent/crates/proto/src/lib.rs](agent/crates/proto/src/lib.rs) (`Protocol::Postgres`, `PostgresFlow`), [agent/src/hub_client.rs](agent/src/hub_client.rs) (`HubPostgresFlow` wire mapping), [agent/src/main.rs](agent/src/main.rs) (console print arm), Go side: [hub/api/models/flow.go](hub/api/models/flow.go) (`PostgresFlow` struct), [hub/api/clickhouse/migrate.go](hub/api/clickhouse/migrate.go) (Phase 33: `pg_query`/`pg_command_tag`/`pg_rows_affected`/`pg_error`/`pg_username`/`pg_database` columns), [hub/api/clickhouse/writer.go](hub/api/clickhouse/writer.go) (INSERT mapping), [hub/api/handlers/flows.go](hub/api/handlers/flows.go) (`/api/v1/flows` SELECT + scan). **Not yet done:** dashboard/Flow Explorer UI surfacing in [hub/web/app/flows/](hub/web/app/flows) (backend is ready — `postgres` field will already appear in the JSON response, just needs a UI column/filter chip), and a live-traffic smoke test against a real `psql` session (only synthetic-byte-buffer unit tests have been run so far — see N5 for why that's now possible on this machine, but a real end-to-end capture test still needs the Mac or a Linux box with a Postgres server). |
...
| N5 | **Rust toolchain (rustup + a locally-extracted gcc-12 linker) added to this dev machine** (2026-08-08), enabling real `cargo build`/`cargo test` for the first time — previously all Rust changes had to be hand-off-verified. No sudo was available and no system C compiler was present, so `gcc-12`/`cpp-12`/`libc6-dev`/`libgcc-12-dev`/`libpcap-dev` (+ their runtime deps `libisl23`/`libmpc3`/`libmpfr6`/`libgmp10`) were fetched via `apt-get download` (download-only, no install) and extracted with `dpkg -x` into `~/local-toolchain/root`, with a handful of real-system `.so` files (`libc.so.6`, `libgcc_s.so.1`, `libm.so.6`, etc.) copied in to satisfy Debian's `libc.so` linker script under `--sysroot`. A wrapper at `~/local-toolchain/bin/cc` (on `PATH` via `.bashrc`) makes plain `cc` — and therefore all of `cargo build`/`cargo test` — work normally. Verified: full `agent` workspace builds clean, `cargo test -p parser` passes 41/41 (30 pre-existing + 11 new Postgres tests). This means future Rust work on this machine no longer needs to be hand-off-verified — only genuinely execution-dependent things (live packet capture, real kernel/eBPF, `cargo tauri dev` GUI) still need the Mac. | — | done, no further action |
...
| **Week 3** | ✅ **N3 done ahead of schedule** — Postgres wire-protocol decoding shipped end-to-end (2026-08-08, see N3/N5 above). Remaining Week 3 focus: §3 eBPF Go/Python TLS probes (the deep BPF work) — feeds directly into the incident-replay/security differentiator. |
| **Week 4** | Flow Explorer UI surfacing for the `postgres` flow field (backend/JSON already there, needs a column/filter chip in [hub/web/app/flows/](hub/web/app/flows)) + a live-traffic smoke test against a real Postgres server (needs Mac or Linux box with Docker, since testcontainers-go is Zscaler-blocked here). Stretch: start a second DB protocol (MySQL is the next highest-value target). Open a v0.8 milestone: fleet-observability verification (week 2) + security differentiator (week 3–4) is a coherent "self-hosted observability + DB/TLS visibility" release story. §2 (Windows agent) and desktop polish (§5.5) move to v0.9, scoped as companion-app improvements once the front door has real users. |

Ship `v0.8.0` when §4's V1–V4 are green in CI **and** N3's Postgres decoder
has a live-traffic smoke test passing end-to-end (capture → decode → hub
ingest → dashboard) — the unit tests are green now, but a real `psql`
session hasn't been captured through the full pipeline yet. That is the
actual "small team self-hosts Nexor and gets more value than Datadog for
