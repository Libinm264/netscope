...
pub fn flow_to_dto(flow: &proto::Flow) -> FlowDto {
    ...
    let (protocol, info) = match &flow.payload {
        Some(FlowPayload::Http(h)) => {
            ...
            let host   = h.request.as_ref()
                .and_then(|r| r.headers.iter().find(|(k, _)| k.eq_ignore_ascii_case("host")))
                .map(|(_, v)| v.as_str()).unwrap_or("");
            let status_suffix = h.response.as_ref()
                .map(|r| format!(" → {}", r.status_code)).unwrap_or_default();
            let latency_suffix = h.latency_ms
                .map(|ms| format!(" ({}ms)", ms)).unwrap_or_default();
            let info = format!("{} {}{}{}{}", method, host, path, status_suffix, latency_suffix);

            let is_error = h.response.as_ref().map(|r| r.status_code >= 400).unwrap_or(false);
            let proto_str = if is_error {
                format!("HTTP {}", h.response.as_ref().map(|r| r.status_code).unwrap_or(0))
            } else {
                "HTTP".to_string()
            };
            ...
            http = Some(HttpFlowDto {
                ...
                req_headers:       h.request.as_ref()
                    .map(|r| r.headers.iter().map(|(k, v)| [k.clone(), v.clone()]).collect())
                    .unwrap_or_default(),
                resp_headers:      h.response.as_ref()
                    .map(|r| r.headers.iter().map(|(k, v)| [k.clone(), v.clone()]).collect())
                    .unwrap_or_default(),
        ...
        Some(FlowPayload::Dns(d)) => {
            let answers = d.answers.iter().map(|a| a.data.as_str()).collect::<Vec<_>>().join(", ");
            ...
            dns = Some(DnsFlowDto {
                ...
                rcode:          d.rcode.clone(),
                answers: d.answers.iter().map(|a| DnsAnswerDto {
                    name:        a.name.clone(),
        ...
        Some(FlowPayload::Tls(t)) => {
            let info = match t.record_type.as_str() {
                ...
                "ServerHello" => format!(
                    "ServerHello {}{}",
                    t.negotiated_version.as_deref()
                        .or(t.chosen_cipher.as_deref()).unwrap_or("?"),
                    t.chosen_cipher.as_deref()
        ...
        Some(FlowPayload::Arp(a)) => {
            let info = format!("{} {} → {} ({})", a.operation, a.sender_ip, a.target_ip, a.sender_mac);
        ...
        None => {
            ...
            let info = format!("{}:{} → {}:{}", flow.src_ip, flow.src_port, flow.dst_ip, flow.dst_port);
            (proto_str, info)
        }
    };
