...
import { Server, Plus, X, Copy, Check, RefreshCw, ExternalLink } from "lucide-react";
import { clsx } from "clsx";
import { fetchAgents, createEnrollmentToken } from "@/lib/api";
import type { Agent, EnrollmentToken } from "@/lib/api";

// ── Status helpers (same as AgentList) ────────────────────────────────────────
...
function CopyButton({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={clsx("p-1 rounded transition-colors", className ?? "hover:bg-white/10 text-slate-500 hover:text-slate-300")}
    >
...
function AddAgentModal({ onClose }: { onClose: () => void }) {
  ...
  const installCmd = token
    ? `curl -sSL "${hubURL}/install?token=${token.token}" | INTERFACE=${iface} sh`
    : "";
          ...
          {step === "form" ? (
            <>
              <p className="text-xs text-slate-400">
                Generate a short-lived enrollment token. Use it with the one-line install command
                to register a new agent without exposing your admin API key.
              </p>
            ...
            <>
              <p className="text-xs text-slate-400">
                Run this command on the target machine. The agent will enroll, download itself, and start capturing.
              </p>

              {/* One-liner */}
              <div>
                <p className="text-xs font-medium text-slate-300 mb-1.5">One-line install</p>
              ...
              {/* Manual command */}
              <div>
                <p className="text-xs font-medium text-slate-300 mb-1.5">Or start manually (after building the agent)</p>
                <div className="relative bg-black/40 border border-white/10 rounded-md px-3 py-2.5 pr-10">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2 rounded-md text-xs border border-white/10 text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]"
                >
                  Generate another
                </button>
                <button
...
export default function AgentsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setAgents((await fetchAgents()).agents ?? []); } catch { /* ignore */ }
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Online if last flow reported within 5 minutes
          </p>
        </div>
      ...
      {loading && agents.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
          ...
          <p className="text-xs text-slate-600">Click <strong className="text-slate-400">Add Agent</strong> to get a one-line install command</p>
          <button
            onClick={() => setShowModal(true)}
